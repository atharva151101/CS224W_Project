...
class HGTEncoder(nn.Module):
    """
    HGT encoder that projects each node type into a shared hidden space
    then applies stacked HGTConv layers.
    """
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train an HGT model for movie rating prediction (node regression)."
    )
    parser.add_argument(
        "--data_path",
        type=str,
